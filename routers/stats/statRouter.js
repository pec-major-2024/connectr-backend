...
statRouter.route("/note")
    .post(async (req, res) => {
        try {
            ...
            console.log("Error in get note's stat route:", { err });
        }
    })
...
statRouter.route("/")
    .get(UserStats, async (req, res) => {
        try {
            ...
            let date_arr = req?.query?.date.split('/');
            const date = new Date(date_arr[2], date_arr[1], date_arr[0]);
            ...
            console.log("Error in get user's stat route:", { err });
        }
    })
